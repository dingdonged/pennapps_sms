...
def getWeather (zip) :
	...
	high_temp = 'High: '+str(1.8(max-273.15)+32)+'F'
	response.close()
...
def incoming_sms():
    ...
    resp.message('Get local weather here. Your zip code?')

    zip = request.values.get('Zip', None)

    zip_resp = MessagingResponse()
    zip_resp.message(getWeather(zip))
